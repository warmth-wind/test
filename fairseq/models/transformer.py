...
class TransformerModel(FairseqEncoderDecoderModel):
    ...
    def forward(
        ...
        encoder_out = self.encoder(
            src_tokens, src_lengths=src_lengths, src_bert_feats=src_bert_feats, alpha=alpha, return_all_hiddens=return_all_hiddens
        )
...
class TransformerEncoder(FairseqEncoder):
    ...
    def __init__(self, args, dictionary, embed_tokens):
        ...
        self.bert_feat_fc = nn.Linear(768, embed_dim)
    ...
    def bert_mixup(self,x ,bert_feats, mix_alpha):
        mixed_x = mix_alpha.unsqueeze(0).unsqueeze(-1) * x + (1 - mix_alpha.unsqueeze(-1)) * bert_feats.unsqueeze(0).repeat(x.size(0), 1, 1)
        return  mixed_x
    ...
    def forward(
        ...
        return self.forward_scriptable(
            src_tokens, src_lengths, src_bert_feats, alpha, return_all_hiddens, token_embeddings
        )
    ...
    def forward_scriptable(
        ...
        # B x T x C -> T x B x C
        x = x.transpose(0, 1)
        src_bert_feats = self.bert_feat_fc(src_bert_feats)
        mixed_x = self.bert_mixup(x, src_bert_feats,alpha)
        x = x + mixed_x
        ...
        for idx, layer in enumerate(self.layers):
            x = layer(
                x, src_bert_feats, idx, encoder_padding_mask=encoder_padding_mask if has_pads else None
            )
            if return_all_hiddens:
