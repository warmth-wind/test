...
class Dictionary:
    ...
    def add_symbol(self, word, n=1, overwrite=False):
        ...
        if word in self.indices and not overwrite:
            ...
            self.count[idx] = self.count[idx] + n #词频加一
            return idx
        else:
    ...
    def finalize(self, threshold=-1, nwords=-1, padding_factor=8):
        ...
        c = Counter(
            dict(
                sorted(zip(self.symbols[self.nspecial :], self.count[self.nspecial :]))  #：zip打包元组，sorted,对可迭代对象排序，返回新列表
            )
        )
    ...
    def _add_file_to_dictionary_single_worker(
        ...
        with open(PathManager.get_local_path(filename), "r", encoding="utf-8") as f:
            size = os.fstat(f.fileno()).st_size  #获得文件大小
            chunk_size = size // num_workers
            offset = worker_id * chunk_size   #offset 偏移量
    ...
    def add_file_to_dictionary(filename, dict, tokenize, num_workers):
        def merge_result(counter):
            for w, c in sorted(counter.items()):
                dict.add_symbol(w, c)      #词频从低到高写进字典
